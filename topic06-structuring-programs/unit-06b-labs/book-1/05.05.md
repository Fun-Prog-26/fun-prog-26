...
is as follows : 
![Structure of default stack structure(my-first-project)](./img/01.png)
...
- Look at the sub folders :
 ...
 - Now, look at the files seen below(files we will not be further examining): 
...
- Next we look at two files that we need to understand. 
 ...
 ![Initial version of package.yaml](./img/12.png)
 You can see from this where everything goes. We only need to look at this file, not *my-first-project.cabal*. The structures are similar, but by being comfortable with the structure of the (simplified) *package.yaml*, we (re) generate the *my-first-project.cabal* whenever we have updated the *package.yaml*. 
 (Note that in this package.yaml, the name is camel case - it should be my-first-project - but it is not important here).
 ...
 but you can skip the **stack build** and go to **stack install** as newer versions check if there's a change and rebuilds if necessary. 

 A sample run looks like: ![structure of default stack structure(my-first-project)](./img/05.png)
